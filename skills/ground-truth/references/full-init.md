...
**Факты собрать ПЕРЕД установкой (D28-стиль):** `git ls-files .claude` (трекается ли `.claude/` — определяет committed vs local хуки, D25); наличие `.github/workflows/*.yml` / `.gitlab-ci.yml` (job добавляется в существующий файл, не создаётся параллельный); наличие `.pre-commit-config.yaml` (запись добавляется, только если файл уже есть); какой test runner реально стоит — для Python не только `pytest.ini`, а любой из: `pytest.ini`, `pyproject.toml` с секцией `[tool.pytest.ini_options]`, `setup.cfg` с секцией `[tool:pytest]`, `tox.ini` с секцией `[pytest]`, иначе fallback — наличие каталога `tests/`; для Go/JS — `go.mod`/`package.json`+jest/vitest как и раньше; для Java — `pom.xml` + `src/test/java`. Определяет `check_kind`-дефолт и bridge-тест.
...
| `ground-truth-sync-check.sh` | `<repo>/.claude/hooks/` (D25 да) или `<repo>/tools/ground_truth/hooks/` (D25 нет) | `chmod +x` |
...
| `settings-hooks.json` | записи `Stop`/`SessionStart`/`PreToolUse` мёржатся в `<repo>/.claude/settings.json` (D25 да) или `.claude/settings.local.json` (D25 нет) | не копировать файлом целиком — мёржить массивы, см. ниже |
...
**settings.json merge:** если D25 → committed, добавить записи в `Stop`, `SessionStart` и `PreToolUse` массивы `<repo>/.claude/settings.json` рядом с уже существующими (не заменять массив); если D25 → local, те же записи в `.claude/settings.local.json`. **pre-commit:** запись добавляется только если `.pre-commit-config.yaml` найден фактом выше — не создаётся с нуля ради этого скилла. **CI:** если `.github/workflows/*.yml` есть — job внутрь него по конвенции репо; если голый CI отсутствует — создаётся `ci-github.yml` как отдельный файл (Phase I.9 не молча принимает отсутствие CI).
