...
`claude` запускать из корня репозитория: хуки и rule-файл ищутся относительно него. Установка выполняется одной командой: `python3 <skill>/pipeline/gt.py i9_install --run run.json --write` (без `--write` — сухой прогон, только план; `--facts` печатает собранные факты и выходит без записи). Стадия сама собирает факты ниже и раскладывает `templates/` по таблице; ручная раскладка построчно по таблице остаётся резервом на случай, когда стадию нельзя запустить.

**Факты собрать ПЕРЕД установкой — командами, а не по памяти:** `git ls-files .claude` (трекается ли `.claude/` — определяет committed vs local хуки, D25); наличие `.github/workflows/*.yml` / `.gitlab-ci.yml` (job добавляется в существующий файл; если workflow несколько и `--ci-file` не задан — создаётся отдельный `.github/workflows/ground-truth.yml`; CI нет, а хост в `git remote get-url origin` содержит `gitlab` (сравнивается только хост, не путь) — создаётся `.gitlab-ci.yml` с `image: python:3.12-slim` у job'а; self-hosted GitLab без `gitlab` в имени хоста так не распознаётся — для него указывать `--ci-file .gitlab-ci.yml`; в репо без Python-раннера строка установки в job — `pip install pyyaml pytest`); наличие `.pre-commit-config.yaml` (запись добавляется, только если файл уже есть); какой test runner реально стоит — для Python не только `pytest.ini`, а любой из: `pytest.ini`, `pyproject.toml` с секцией `[tool.pytest.ini_options]`, `setup.cfg` с секцией `[tool:pytest]`, `tox.ini` с секцией `[pytest]`, иначе fallback — каталог `tests/`, в котором есть `*.py` (каталог с одними `*.spec.ts` Python-раннером не считается); для Go — `go.mod`; для JS — `package.json` с jest/vitest в корне или на глубине до 2 (без `node_modules`), а `package.json` без раннера печатает `WARN js present, runner not detected -> bridge skipped, use a probe`; для Java — `pom.xml` + `src/test/java`. Определяет `check_kind`-дефолт и bridge-тест.
...
| `ground-truth.rule.md` | `<repo>/.claude/rules/ground-truth.md` | без `paths:`-frontmatter, always-on (D35); плейсхолдер `{{PY}}` → `.venv/bin/python`, если файл есть, иначе `python3`; venv в другом месте (`venv/`, `env/`, poetry, uv) — экспортировать `GT_PYTHON=<путь к python>`: его читают хуки и TS-мост |
