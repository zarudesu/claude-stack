...
Матрица не бралась с потолка — периодически проверяется replay-эвалом на «золотых» PR: репозиторий клонируется в изолированную копию без доступа к истории задачи (чтобы агент не подсказывал сам себе решение), на каждом коммите гоняется агент нужного тира, дифф сравнивается с реальным человеческим коммитом через jaccard-похожесть изменённых строк. Один такой прогон показал 5 из 9 пустых диффов у haiku на кодовых правках — по факту агент не делал изменений вообще. Вывод: haiku убран из маршрутизации кода целиком, остаётся только для чтения/поиска/аудита.
